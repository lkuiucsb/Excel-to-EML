...
The R code reads the metabase in excel format, uses EML package in R, and generates an EML file in xml format. Because the R functions navigate among different files based on their relative locations among various folders, the name and location of the folder should not be changed (unless you are confident to change the path in the R functions). 

Several steps to make sure the code could run smoothly:
